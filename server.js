...
(function() {
  var Crypto, Fs, Http, Https, Path, QueryString, Url, accepted_image_mime_types, camo_hostname, current_connections, debug_log, default_security_headers, error_log, finish, four_oh_four, hexdec, keep_alive, logging_enabled, max_redirects, port, process_url, server, shared_key, socket_timeout, started_at, total_connections, version,
  ...
  process_url = function(url, transferredHeaders, resp, remaining_redirects) {
    ...
    if (url.host != null) {
      ...
      srcReq = Protocol.get(requestOptions, function(srcResp) {
        var contentType, contentTypePrefix, eTag, expiresHeader, is_finished, lastModified, newHeaders, newUrl, origin;
        is_finished = true;
